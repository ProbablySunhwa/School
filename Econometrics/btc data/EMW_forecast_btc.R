...
EMW_forecast_btc <- function(df.btc, df.z = NULL, method = "OLS") {
  ...
  #colnames(df.z) <- names
  m <- dim(df.btc)[1] # end with 235 / or 340 huh values of 21 different exchange rates
  ...
  for (horizon in 1:4) {
    ...
    for (j in 1:bound.test) {
      ...
      f_fc <- factanal(X_fc,factors = 1) # this is the entire factor analysis function, choose r=2
